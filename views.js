...
var LoginView = Backbone.View.extend ({
	...
	nameOfFunction: function() {
		...
		if(userValid.attributes.password == certainUserPassword) {
			console.log("Success");
			this.remove(); //this removes the loginView contents from the page
			var userView = new UserView({collection: app.issues}); //this allows us to access the 
			//userView. We had to create a new instance of it, and it has to access a collection in order to work (data).
			////The data that it needs to access is in app.issues (which is located in your main.js file).
			userView.render(); //this renders the userView and allows us to see it in the browser
			}
		else {
			alert("Nope");
		};
